...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* flat video overlay: plays on top of the card (outside the 3D transform -> works on iOS) */
  .flip-video {
    position: absolute; inset: 0; z-index: 5; width: 100%; height: 100%; object-fit: cover;
    border: 1px solid var(--hair); border-radius: 6px; background: var(--paper-2);
    opacity: 0; visibility: hidden; transition: opacity .4s ease, visibility .4s;
  }
  .flip-video.is-on { opacity: 1; visibility: visible; }
  /* multi-image input: 3 reference photos shown whole on a clean board (no weird crops) */
  .flip-collage {
    position: absolute; inset: 0; display: none; gap: 6px; padding: 6px; background: var(--paper-2);
    grid-template-columns: 0.78fr 1fr; grid-template-rows: 1fr 1fr;
  }
  .flip-collage .g-cell {
    background: #fff; border: 1px solid var(--hair); border-radius: 3px; overflow: hidden;
    display: flex; align-items: center; justify-content: center; padding: 5px; min-width: 0; min-height: 0;
  }
  .flip-collage .g-cell:first-child { grid-row: 1 / 3; }   /* tall reference fills the left column */
  .flip-collage img { max-width: 100%; max-height: 100%; width: auto; height: auto; object-fit: contain; display: block; }
  .flip-tag {
  ...
  /* single side-by-side example (no flip): same card box, video letterboxed, no flip affordance */
  .flip.is-single { cursor: default; }
  .flip.is-single .flip-video { object-fit: contain; }
    ...
    <div class="gallery" id="gallery">
      <div class="g-main">
        <div class="flip" id="flip" role="button" tabindex="0" aria-label="Flip between input and output">
          ...
          <!-- flat video overlay (NOT inside the 3D transform, so it plays on iOS/mobile) -->
          <video id="g-vid" class="flip-video" muted loop playsinline webkit-playsinline preload="metadata" aria-hidden="true"></video>
        </div>
...
const vid = $("#g-vid");          // one flat video player, reused for every example
vid.muted = true; vid.playsInline = true;
let active = 0;
let flipped = false;

// keep retrying play() until the (muted, inline) clip actually starts -- covers buffering
// races and flaky first-gesture plays on mobile. is-on means it should be playing.
function ensurePlay(tries) {
  if (!vid.classList.contains("is-on")) return;
  vid.play().catch(() => {});
  if (vid.paused && tries > 0) setTimeout(() => ensurePlay(tries - 1), 220);
}
vid.addEventListener("canplay", () => ensurePlay(2));
vid.addEventListener("loadeddata", () => ensurePlay(2));

// the flat overlay plays when: a single (no-flip) example is shown, OR a flip example is on its output.
// play() is called synchronously inside the click gesture, which mobile Safari requires.
function syncVideo() {
  const a = assets[active];
  const show = a.single || (a.video && flipped);
  if (show) {
...
function renderGallery(i) {
  ...
  // point the single flat video player at this example's clip (single OR flip output)
  const src = a.single || a.video || "";
  if (src && vid.getAttribute("src") !== src) vid.src = src;
  if (!a.single) {
    // input front face: a 3-photo collage, or a single image
    if (a.inImgs) {
      collage.innerHTML = a.inImgs.map((k) => `<div class="g-cell"><img src="${IMAGES[k]}" alt="${a.name} — input" draggable="false"></div>`).join("");
    ...
    // image-output fallback (no current asset uses it, but keep the path)
    if (!a.video) {
